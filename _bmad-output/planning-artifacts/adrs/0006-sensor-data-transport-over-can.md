...
**Proposed — re-keyed by ADR-0007 (2026-06-06).** Under the flat-`node_id` model, sensor frames
are keyed by **`node_id`** (`[CAT_SENSOR][node_id]`, standard 11-bit) with `measurement_type` +
value in the **payload**; the room is derived centrally from `node_id`. The category,
