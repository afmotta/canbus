...
**Proposed — partly revised by ADR-0007 (2026-06-06).** Two micro-decisions here flip under
the flat-`node_id` model: **D1 reverses** — `button`/`event` move **back to the payload**
(centralization removed the in-ID filtering rationale), and **D3** priority sub-ordering is now
by `node_id` within a category. The ADR's *core* conclusion strengthens: the KNX
individual-address + central-function-map model (Hypothesis A) is now fully adopted
(`node_id` = individual address; central map = function). The rest stands.

This ADR is a **consolidating record**: it does not introduce a new architecture, it closes
out a critique whose conclusions are otherwise scattered across ADR-0001/0002/0003, and it
makes the few micro-decisions that were never explicitly recorded.
